...
**/
// (1).1  --
// 只要是静态变量，即使是局部的，也不存储在栈中，而是即存储在静态区域中，并依据其是否初始化，分别存储在BSS段和DATA段
static int             /*bss_*/
    unini_glo_sta_var; //  静态未初始化全局变量，虽然系统会默认初始化为0，但仍然存放在BSS区
static int /*data_*/ ini_glo_sta_var =
    10; //  静态初始化全局变量，存放在RW DATA区

// (1).2  --
// 只要是全局变量，即存储在静态区域，并依据其是否初始化，分别存储在BSS段和DATA段
int /*bss_*/ unini_glo_var; //  未初始化全局数据存储在BSS数据区
int /*data_*/ ini_glo_var = 42; //  初始化全局数据存储在RW DATA数据区

// (1).3  --  全局常量，其本身是全局变量，即存储在静态区域, 同(1).2
const int unini_glo_cnt_var; //  未初始化全局常量[不安全]， 自动初始化为0，
                             //  但仍然存放在BSS区
const int ini_glo_cnt_var = 10; //  初始化全局常量，存储在常量区
// 对于常量我们需要注意的问题在于，他们并不像我们期望的那样存储在常量区(RO
// data)， 常量区只用于存储初始化好的全局常量以及字符串变量本身（不是是指针）
// 局部常量作为局部量仍然存储与栈中
...
int main(void) {
  ...
  //  (2).3  --
  //  指针常量和常量指针，其本质还是局部变量或者局部常量，存储在栈中，同(2).1
  //  (2).2
  ...
  printf("------------------------------------------------------\n");
  printf(".Bss Location:\n");
  SHW_VAR_ADR("unini_glo_sta_var",
              unini_glo_sta_var); //  全局未初始化静态变量, 在BSS段
  SHW_VAR_ADR("unini_sta_var", unini_sta_var); //  未初始化静态变量，在BSS段
  SHW_VAR_ADR("unini_glo_cnt_var",
              unini_glo_cnt_var); //  全局未初始化常量，在BSS段
  SHW_VAR_ADR("unini_glo_var", unini_glo_var); //  全局未初始化变量在, BSS段
  SHW_VAR_ADR("unini_glo_sta_cnt_var",
              unini_glo_sta_cnt_var); //  全局未初始化静态常量，在BSS段
  printf("------------------------------------------------------\n\n");

  // RW DATA段 -- 可读写已初始化数据段
  printf("------------------------------------------------------\n");
  printf(".Data Location:\n");
  SHW_VAR_ADR("ini_glo_sta_var",
              ini_glo_sta_var); //  全局初始化静态变量存储在RW data区域
  SHW_VAR_ADR("ini_glo_var", ini_glo_var); //  全局初始化变量存储在RW data

  SHW_VAR_ADR("ini_sta_var",
              ini_sta_var); //  局部初始化静态变量存储在RW data区域

  SHW_VAR_ADR("ini_glo_sta_cnt_var",
              ini_glo_sta_cnt_var); //  全局静态已初始化常量，存储在RW data区域
  printf("------------------------------------------------------\n\n");
  ...
  SHW_VAR_ADR(
      "ini_glo_cnt_var",
      ini_glo_cnt_var); // 初始化全局常量，同字符串面变量一样，位于文本区，即常量区
  ...
  //  局部常量(不管初始化没有)存储在栈中
  SHW_VAR_ADR("unini_cnt_var",
              unini_cnt_var); // 未被初始化的局部常量，不安全，存储在栈中
  SHW_VAR_ADR("ini_cnt_var", ini_cnt_var); // 局部常量，存储在栈中

  //  指针常量和常量指针，其本质还是局部变量或者常量，存储在栈中
  SHW_VAR_ADR(
      "p_cnt_var",
      p_cnt_var); //  该指向常量的指针，其本身其实是一个（初始化的）局部变量[同ini_var],
                  //  存储在栈中
  SHW_VAR_ADR(
      "cnt_p_var",
      cnt_p_var); //  该指针常量，其本身其实是一个初始化的局部常量[同ini_cnt_var]，
                  //  存储在栈中
  SHW_VAR_ADR(
      "cnt_p_cnt_var",
      cnt_p_cnt_var); //  该指向常量的指针常量作为一个初始化的局部常量，存储在栈中

  SHW_POT_ADR("str_array",
              str_array); //  字符串数组，相当于初始化的局部变量，保存在栈中
  SHW_VAR_ADR(
      "str_array",
      str_array); //  指针str2本身在栈中，其地址本身，就是字符串数组的地址
